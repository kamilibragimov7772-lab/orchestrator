...
subprocess.run([FF, "-y", "-hide_banner", "-loglevel", "error", "-f", "lavfi", "-i",
                "color=c=0x0B1020:s=%dx%d:d=%.2f:r=%d" % (W, H, INTRO, FPS), "-vf", vf_intro,
                "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", intro], check=False)
...
subprocess.run([FF, "-y", "-hide_banner", "-loglevel", "error", "-f", "lavfi", "-i",
                "color=c=0x111A2E:s=%dx%d:d=%.2f:r=%d" % (W, H, OUTRO, FPS), "-vf", vf_outro,
                "-c:v", "libx264", "-preset", "veryfast", "-crf", "20", outro], check=False)
...
subprocess.run([FF, "-y", "-hide_banner", "-loglevel", "error", "-i", silent, "-i", CLEAN,
                ...
                "-c:a", "aac", "-b:a", "192k", OUTFILE], check=False)
print("DONE:", OUTFILE, "exists:", os.path.exists(OUTFILE))
